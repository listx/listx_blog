...
To avoid this, you can use the alternate expression `rand_excess = (RAND_MAX % n) + 1;` for setting `rand_excess`:
...
To get around this, we can do one more modulo operation `rand_excess = ((RAND_MAX % n) + 1) % n;` to avoid the case where `rand_excess` could equal `n`:
